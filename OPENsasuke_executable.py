def pass_fail_Calculation(): # This is the core/heart of the simulation, so to speak.
    ...
    def fuck():
        ...
        obstat = set(obstacle_data.keys()) if isinstance(obstacle_data, dict) else set()
        
        print(obstat)
        
        global compstat
        
        compstat = set(competitor_data.keys()) if isinstance(competitor_data, dict) else set()  
        
        print(compstat)

        global common
        
        common = obstat & compstat
    ...
    def shit():
        ...
        for val in common:
            x = val
            y = val
            val1 = competitor_data[x]
            
            val2 = obstacle_data[y]
